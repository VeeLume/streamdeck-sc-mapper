...
pub fn load_translations_cached_from_bindings(
    ...
    let bytes = match fs::read(&global_ini_path) {
        Ok(b) => b,
        Err(e) => {
            warn!(logger, "read {}: {}", global_ini_path.display(), e);
            return HashMap::new();
        }
    };
    ...
    if let Ok(bytes) = serde_json::to_vec(&cache) {
        if let Err(e) = fs::write(&cache_path, bytes) {
            warn!(logger, "write cache {}: {}", cache_path.display(), e);
        }
    }
...
fn collect_translation_keys_from_bindings(bindings: &ActionBindings) -> HashSet<String> {
    ...
    // Important: for every "@foo.bar" also keep "foo.bar" so we match INI keys with/without '@'
    // (we do this in-place to avoid extra allocation passes).
...
fn decode_text(bytes: &[u8], path: &Path, logger: &Arc<dyn ActionLog>) -> String {
