...
/// A single `<actionmap>` block with its actions.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ActionMap {
    pub name: Arc<str>,
    pub version: u32,

    /// Kept as `String` so we can `as_deref()` cleanly in `get_label`.
    pub ui_label: Option<String>,
    pub ui_category: Option<String>,

    /// Keyed by `action_name` (Arc<str>) to match `ActionBinding.action_name`.
...
impl ActionMap {
    /// Preferred label resolution for display:
    /// 1) `UILabel`, 2) `UICategory`, 3) map `name`.
    ...
    /// Parse an `<actionmap>` node (and all its `<action>` children).
    ///
    /// Returns the `ActionMap` plus a list of per-action parse errors (bind or action errors).
    pub fn from_node(
        node: Node,
        ...
        let ui_category = node
            ...
            .filter(|s| !s.is_empty())
            .map(str::to_string)
            // fallback from static map by actionmap name
