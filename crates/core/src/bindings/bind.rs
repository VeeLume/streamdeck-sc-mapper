//! Bind representation + parsing (core-friendly).
//!
//! Notes:
//! - This module re-exports `Key` and `MouseButton` so downstreams can depend
//!   only on `streamdeck_sc_core` instead of `streamdeck-lib` directly.
//! - Parsing accepts a liberal set of aliases and device prefixes to match SC XML.
...
pub use streamdeck_lib::input::{Key, MouseButton};
...
pub enum BindOrigin {
    #[default]
    /// Defaults + user-provided rebinds
    User,
    /// Produced by BindGenerator
    Generated,
}

/// Main part of a bind (the "key" side).
...
/// A single input bind: (modifiers) + main key, plus an optional activation-mode index.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Bind {
    pub main: Option<BindMain>,
    pub modifiers: HashSet<Key>,

    /// Index into ActivationArena (external to this module).
...
impl fmt::Display for BindParseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            BindParseError::TooManyMainKeys { input, main_keys } => write!(
                f,
                "too many main keys in '{input}': {}",
                main_keys.join(", ")
            ),
            BindParseError::NoInput => write!(f, "no valid input tokens"),
        }
    }
}
...
impl Hash for Bind {
    fn hash<H: Hasher>(&self, state: &mut H) {
        ...
        // Deterministic hashing of modifiers WITHOUT requiring Key: Ord
...
impl Bind {
    ...
    ///   "lctrl+f", "LShift+A", "np_1", "kb1_lctrl+f", "" (empty → explicit unbind)
    ///
    ...
    pub fn from_string(
        ...
        // Strip known device prefixes (keep token semantics like "np_1")
        ...
        for seg in segments {
            ...
            // 2) Mouse axes / HMD axes
            if let Some(axis) = s
                .strip_prefix("maxis_")
                .or_else(|| s.strip_prefix("mouse_axis_"))
            {
                main_keys.push(BindMain::MouseAxis(axis.into()));
                continue;
            }
            if let Some(hmd) = s.strip_prefix("hmd_") {
                main_keys.push(BindMain::HMD(hmd.into()));
                continue;
            }
        ...
        match main_keys.len() {
            ...
            0 if modifiers.len() == 1 => {
                let mainkey = *modifiers.iter().next().ok_or(BindParseError::NoInput)?;
                Ok(Bind {
...
impl FromStr for Bind {
    ...
    /// Parse with `activation_mode_idx = None`.
...
fn strip_device_prefix(s: &str) -> &str {
    // NOTE: SC device instance prefixes vary; keep this liberal but finite.
    const PREFIXES: &[&str] = &[
        "kb1_", "kb2_", "kb_", "mo1_", "mo2_", "mo_", "gp1_", "gp2_", "gp_", "js1_", "js2_", "js_",
    ];
    for p in PREFIXES {
